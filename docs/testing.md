...
## Validated cases
- `ap_front_plate.json` produced `penetration`, hit `hull_front`, generated fragment paths, damaged the authored `ammo_rack` volume, and incapacitated the authored `driver` hitbox.
...
- Crew and module hit volumes are still axis-aligned boxes; authored sizes improve fidelity, but posture, rotation, and occlusion are still coarse.
